...
def monitorar():
    ...
    logMonitor.info("horario,memoria(%),cpu(%),Block-use(bytes),Net-in(bytes),Net-out(bytes)")
    ...
    while(cont == True):
        ...
        valor = str(mem) + "," + str(cpu) +"," + str(usoDisco)+ "," +str(entradaNET)+ "," + str(saidaNET)  
        logMonitor = logging.getLogger('logCPU_MEM')
...
print("Monitorando")
print("indice,memoria(%),cpu(%),Block-use(bytes),Net-in(bytes),Net-out(bytes)")
