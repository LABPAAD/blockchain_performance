...
if [ -z "$1" ]; then
  echo "Usage: $0 <new_batch_timeout>"
  exit 1
fi

# Define o novo timeout do lote
NEW_BATCH_TIMEOUT=$1
...
BATCHTIMEOUTPATH=".channel_group.groups.Orderer.values.BatchTimeout.value"
echo "Current Batch Timeout:"
docker exec -e BATCHTIMEOUTPATH=$BATCHTIMEOUTPATH cli sh -c 'jq "$BATCHTIMEOUTPATH" config.json'

# Passo 4: Atualizando o valor
echo "Updating Batch Timeout to $NEW_BATCH_TIMEOUT..."
docker exec -e BATCHTIMEOUTPATH=$BATCHTIMEOUTPATH cli sh -c "jq \"$BATCHTIMEOUTPATH.timeout = \\\"$NEW_BATCH_TIMEOUT\\\"\" config.json > modified_config.json"

# Verificar o conteúdo modificado
docker exec -e BATCHTIMEOUTPATH=$BATCHTIMEOUTPATH cli sh -c 'jq "$BATCHTIMEOUTPATH" modified_config.json'
...
echo "Batch timeout modification completed successfully."
